...
while true; do
	...
	for i in `eval $e {1..$R}`; do
		c=`printf "%s" $(shuf -n1 -e m i n i s h e l l M I N I S H E L L ═ ║ ╔ ╗ ╚ ╝ █ ░)`
		p_char="\033[$[i-1];${j}H\033[32m$c"
		curr_char="\033[$i;${j}H\033[37m$c"
		char_in_banner=$(perl -C -ne 'BEGIN{$row='$(($R - $i + 1))'; $col='$j'} if($. == $row) {print substr($_, $col, 1); exit}' <<< "$REVERSED_BANNER")
		...
		if [[ -n "$char_in_banner" && "$char_in_banner" != " " ]] && [ $random_chance -le 100 ]; then
			...
			char_in_banner=$(tac "minishell_banner.txt" | perl -C -ne 'BEGIN{$row='$(($R - $i))'; $col='$j'} if($. == $row) {print substr($_, $col, 1); exit}')
			if [[ -n "$char_in_banner" && "$char_in_banner" != " " ]] && [ $random_chance -le 100 ]; then
				$e $curr_char
			else
				$e $p_char$curr_char
